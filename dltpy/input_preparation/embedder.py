...
# class IdentifiersIterator(HelperIterator):
#     def __init__(self, param_df: pd.DataFrame, return_df: pd.DataFrame) -> None:
#         self.param_df = param_df
#         self.return_df = return_df
...
class EmbeddingTypeWriter(Embedder):

    # def train_identifier_model(self):
    #     """
    #     Trains a W2V model for Identifiers
    #     """

    #     self.train_model(IdentifiersIterator(self.param_df, self.return_df), config.W2V_MODEL_ID_DIR)
    ...
    def train_token_model(self):
        ...
        self.train_model(TokenIterator(self.param_df, self.return_df), config.W2V_MODEL_TOKEN_DIR)
    ...
    def train_comment_model(self):
        ...
        self.train_model(CommentIterator(self.param_df, self.return_df), config.W2V_MODEL_COMMENTS_DIR)
